#!/usr/bin/env python3
"""
BCI2b SMOTE 风格数据增强三场景评估（单独版本，不覆盖 BCI2a）
...
- 结果保存到 `outputs/results/smote_bci2b_all_scenarios.json`
"""
...
def smote_augmentation_bci2b(
    X_train: np.ndarray,
    y_train: np.ndarray,
    n_samples_per_class: int,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    针对 BCI2b 的 SMOTE 风格数据增强（在通道×时间空间内做线性插值）。
    ...
    for c in classes:
        ...
        for _ in range(n_samples_per_class):
            ...
            if len(class_data) > 1:
                # 选一个不同的邻居
                neighbor_indices = [i for i in range(len(class_data)) if i != idx]
                neighbor = class_data[np.random.choice(neighbor_indices)]
...
def within_subject_test_bci2b(X: np.ndarray, y: np.ndarray, subjects: np.ndarray) -> Dict:
    """BCI2b 被试内测试 + SMOTE 增强。"""
    print("\n" + "=" * 70)
    print("1. BCI2b Within-Subject 测试（SMOTE 增强）")
    print("=" * 70)
    ...
    for idx, subj_id in enumerate(unique_subjects):
        ...
        gen_X, gen_y = smote_augmentation_bci2b(X_train, y_train, samples_per_class)
...
def cross_session_test_bci2b(X: np.ndarray, y: np.ndarray, subjects: np.ndarray, sessions: np.ndarray) -> Dict:
    """BCI2b 跨会话测试（T->E）+ SMOTE 增强。"""
    print("\n" + "=" * 70)
    print("2. BCI2b Cross-Session 测试（T->E，SMOTE 增强）")
    print("=" * 70)
...
def cross_subject_test_bci2b(X: np.ndarray, y: np.ndarray, subjects: np.ndarray, sessions: np.ndarray) -> Dict:
    """BCI2b 跨被试测试（LOSO，T-only）+ SMOTE 增强。"""
    print("\n" + "=" * 70)
    print("3. BCI2b Cross-Subject 测试（LOSO, T-only，SMOTE 增强）")
    print("=" * 70)
...
def main() -> None:
    ...
    save_path = "outputs/results/smote_bci2b_all_scenarios.json"
    with open(save_path, "w", encoding="utf-8") as f:
        json.dump(final_results, f, indent=2, ensure_ascii=False)

    print(f"\n结果已保存到: {save_path}")
    print("\n" + "=" * 70)
    print("BCI2b SMOTE 评估完成")
    print("=" * 70)
