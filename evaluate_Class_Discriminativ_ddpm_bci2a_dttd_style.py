...
# -*- coding: utf-8 -*-
"""
BCI2a: DTTD-style evaluation (NO normalization, raw data * 1e6 uV)

与原始 evaluate_Class_Discriminativ_ddpm_bci2a.py 的关键区别:
...
配合训练脚本: train_class_discriminative_ddpm_bci2a_dttd_style.py
配合基线脚本: train_baselines_dttd_style.py --dataset bci2a
"""
...
# ============================================================================
# DTTD-style EEGNet Classifier
# ============================================================================

class DTTD_EEGNetClassifier(nn.Module):
    """EEGNet classifier matching DTTD project architecture."""
...
def load_bci2a_data(data_root="data/processed/BCI2a"):
    """Load BCI2a data — DTTD style: scale to uV, no normalization."""
    X = np.load(f'{data_root}/X.npy').astype(np.float32) * DATA_SCALE
    y = np.load(f'{data_root}/y.npy').astype(np.int64)
    y = y - y.min()
    mask = y < NUM_CLASSES
    X, y = X[mask], y[mask]

    n_subjects, n_sessions = 9, 2
    ...
    print(f"BCI2a (DTTD style): {X.shape}, range=[{X.min():.4f}, {X.max():.4f}] uV, "
...
def load_ddpm_bci2a(checkpoint_path):
    """Load DTTD-style DDPM (trained on uV data, no normalization)."""
...
def load_baseline_bci2a(model_name):
    """Load DTTD-style baseline model."""
...
CACHE_DIR = "outputs/results/cache_bci2a_dttd_style"
...
def run_within_subject(X, y, subjects, ddpm, baseline_models, guidance_scale):
    print("\n" + "=" * 70)
    print("1. BCI2a Within-Subject Test — DTTD Style (uV space)")
    print("=" * 70)
...
def run_cross_session(X, y, subjects, sessions, ddpm, baseline_models, guidance_scale):
    print("\n" + "=" * 70)
    print("2. BCI2a Cross-Session Test (T->E) — DTTD Style (uV space)")
    print("=" * 70)
...
def run_cross_subject(X, y, subjects, sessions, ddpm, baseline_models, guidance_scale):
    print("\n" + "=" * 70)
    print("3. BCI2a Cross-Subject Test (LOSO, T-only) — DTTD Style (uV space)")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="BCI2a DTTD-Style Evaluation (uV space, no normalization)")
    parser.add_argument("--data_root", type=str, default="data/processed/BCI2a")
    parser.add_argument("--ddpm_ckpt", type=str, default="checkpoints/bci2a_dttd_style/best_ddpm.pt")
    parser.add_argument("--guidance_scale", type=float, default=2.0)
    args = parser.parse_args()

    print("=" * 70)
    print("BCI2a DTTD-Style Evaluation (NO normalization, raw data * 1e6 uV)")
    print("=" * 70)
    ...
    X, y, subjects, sessions = load_bci2a_data(args.data_root)

    # Load DTTD-style DDPM
    ddpm = load_ddpm_bci2a(args.ddpm_ckpt)
    ...
    for name in ['cvae', 'wavegan', 'cond_ddpm', 'braindiff', 'eegdiff', 'diffeegbooth']:
        ...
        if model is not None:
            ...
            print(f"  Loaded {name} for BCI2a (DTTD style)")
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("BCI2a DTTD-Style Results Summary")
    print("=" * 70)
    ...
    out_path = "outputs/results/all_methods_bci2a_kappa_dttd_style.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"dataset": "BCI2a", "results": results}, f, indent=2, ensure_ascii=False)
