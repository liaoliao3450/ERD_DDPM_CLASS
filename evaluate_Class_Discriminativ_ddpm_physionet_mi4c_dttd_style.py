...
# -*- coding: utf-8 -*-
"""
PhysioNet MI4C: DTTD-style evaluation (NO normalization, raw data * 1e6 uV)

与原始 evaluate_Class_Discriminativ_ddpm_physionet_mi4c.py 的关键区别:
...
配合训练脚本: train_class_discriminative_ddpm_physionet_dttd_style.py
配合基线脚本: train_baselines_dttd_style.py --dataset physionet
"""
...
# ============================================================================
# DTTD-style EEGNet Classifier (64-channel PhysioNet)
# ============================================================================

class DTTD_EEGNetClassifier(nn.Module):
    """EEGNet classifier matching DTTD project architecture (PhysioNet: 64 channels)."""
...
def load_physionet_data(data_root="data/processed/PhysioNetMI4C"):
    """Load PhysioNet MI4C data — DTTD style: scale to uV, no normalization."""
    from data_loader_physionet_mi4c import load_physionet_mi4c_data as _load
    X, y, subjects, sessions, _ = _load(data_root, standardize=False)
    ...
    print(f"PhysioNet (DTTD style): {X.shape}, range=[{X.min():.4f}, {X.max():.4f}] uV, "
          f"classes: {[sum(y==i) for i in range(NUM_CLASSES)]}, subjects: {len(np.unique(subjects))}")
...
def load_ddpm_physionet(checkpoint_path):
    """Load DTTD-style DDPM for PhysioNet (trained on uV data, no normalization)."""
...
def load_baseline_physionet(model_name):
    """Load DTTD-style baseline model for PhysioNet."""
...
CACHE_DIR = "outputs/results/cache_physionet_dttd_style"
...
def run_within_subject(X, y, subjects, ddpm, baseline_models, guidance_scale):
    print("\n" + "=" * 70)
    print("1. PhysioNet Within-Subject Test — DTTD Style (uV space)")
    print("=" * 70)
    ...
    for idx, subj_id in enumerate(unique_subjects):
        ...
        if len(np.unique(y_subj)) < NUM_CLASSES or len(X_subj) < 10:
            print(f"  Skipping: insufficient data ({len(X_subj)} samples)")
            continue
...
def run_cross_session(X, y, subjects, sessions, ddpm, baseline_models, guidance_scale):
    print("\n" + "=" * 70)
    print("2. PhysioNet Cross-Session Test (T->E) — DTTD Style (uV space)")
    print("=" * 70)
...
def run_cross_subject(X, y, subjects, sessions, ddpm, baseline_models, guidance_scale):
    print("\n" + "=" * 70)
    print("3. PhysioNet Cross-Subject Test (LOSO, T-only) — DTTD Style (uV space)")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description="PhysioNet DTTD-Style Evaluation (uV space, no normalization)")
    parser.add_argument("--data_root", type=str, default="data/processed/PhysioNetMI4C")
    parser.add_argument("--ddpm_ckpt", type=str, default="checkpoints/physionet_dttd_style/best_ddpm.pt")
    parser.add_argument("--guidance_scale", type=float, default=2.0)
    args = parser.parse_args()

    print("=" * 70)
    print("PhysioNet DTTD-Style Evaluation (NO normalization, raw data * 1e6 uV)")
    print("=" * 70)
    ...
    X, y, subjects, sessions = load_physionet_data(args.data_root)

    # Load DTTD-style DDPM
    ddpm = load_ddpm_physionet(args.ddpm_ckpt)
    ...
    for name in ['cvae', 'wavegan', 'cond_ddpm', 'braindiff', 'eegdiff', 'diffeegbooth']:
        ...
        if model is not None:
            ...
            print(f"  Loaded {name} for PhysioNet (DTTD style)")
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("PhysioNet DTTD-Style Results Summary")
    print("=" * 70)
    ...
    out_path = "outputs/results/all_methods_physionet_mi4c_kappa_dttd_style.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"dataset": "PhysioNet", "results": results}, f, indent=2, ensure_ascii=False)
